...
Along with the minimal class diagram this chapter will display the state machine and refined sequence diagrams.
The design patterns used and the class sections that contain the OCL statements will be identified.
\section{Overview}
...
\begin{description}
	\item [BlueJay] is a social media platform that allows users to post texts and images in a safe and secure environment.
	\item [StarUML] StarUML is a software tool used to create UML diagrams.
	\item [SM User] Social media user; the user who is logged into the system.
	\item [Three Tier] Three-tier architecture is a client-server software architecture pattern in which the user interface (presentation), functional process logic ("business rules"), computer data storage and data access are developed and maintained as independent modules, most often on separate platforms.
	\item [CSS] Cascading Style Sheets is a style sheet language used for describing the presentation of a document written in a markup language like HTML.
	\item [SHA-256] Cryptographic hash function designed by the National Security Agency (NSA), widely used in various security applications.
\end{description}
